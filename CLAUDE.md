...
3. **[어시스턴트가 함]** 저장은 **"Drive에 프로젝트 저장" 버튼을 클릭하지 말고, 에디터에 포커스가 있는 상태에서 `computer` 툴의 `key` 액션으로 `ctrl+s`를 누른다** — 저장 버튼 클릭은 이 세션에서 두 번 실패했다(반영이 안 됨). `ctrl+s` 직후 곧바로 새로고침하면 "저장 중..."이 끝나기 전이라 반영이 안 된 것처럼(구버전으로) 보일 수 있었다(2026-07-29 두 번째 재배포 때 실제로 한 번 이렇게 헷갈렸다) — `ctrl+s` 후 2~3초는 기다렸다가 새로고침할 것. 저장했다고 믿지 말고, **반드시 페이지를 새로고침한 뒤 모델 내용을 로컬 파일과 문자열 그대로 비교**해서(`remote === local`) 실제로 서버에 반영됐는지 검증한다. `false`가 나오면 실패로 간주하고 파일 탭 클릭부터 다시 반복할 것(재시도하면 대개 된다) — 검증 전엔 사용자에게 "저장했다"고 보고하지 않는다.
