...
**마지막 갱신**: 2026-08-08 · 세션 15 (**현황 암호** — 진행 상황 화면 전용의 낮은 권한 암호, 아래 "세션 15" 절 참고 — ⚠️ **`Code.gs` 재배포 필요**: 새 액션 `setViewPassword`)
...
⚠️ **다음에 할 일**: `Code.gs` 재배포(15 → 16). 안 하면 현황 암호 설정이 "알 수 없는 action: setViewPassword"로 실패한다. 편집 키로 들어가는 기존 경로는 영향 없다.
