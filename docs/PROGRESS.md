...
**마지막 갱신**: 2026-08-18 · 세션 16 (**학생 진입 화면에서 현황판으로 가는 작은 링크** — 아래 "세션 16" 절)

### 세션 16 — 폰으로 현황판에 빨리 들어가기 (2026-08-18)

**배경**: 사용자가 "수업현황보기로 접속할 때 수업코드도 넣어야 해서 빠르게 접속하기 힘들다. 폰으로 주로 현황판을 보는데 QR 같은 건 없나"라고 물었다.

QR 링크에 현황 암호를 담는 안, 서버 발급 티켓 안 등을 제시했으나 **사용자가 고른 것은 더 단순한 방향**이다 — *"학생 QR코드로 접속했을 때, 홈의 「관리자」 버튼처럼 화면 아래쪽에 작게 뜨게 하되 현황 암호를 묻는 것으로."*

교사도 학생용 QR을 그대로 찍으면 **수업 코드가 주소에 들어오므로**, 거기서 한 번 누르면 코드 입력이 사라진다. 남는 건 현황 암호 한 번뿐이고 그것도 기기에 저장된다.

- `player/EntryScreen.tsx` — `liveCode` prop이 있을 때만 맨 아래에 「수업 현황 보기」를 띄운다. 홈의 「관리자」와 **같은 자리·같은 크기**(`text-xs text-neutral-400 underline` + `tap-target`).
- `player/Player.tsx` — `mode === 'live' && !isTest`일 때만 `liveCode`를 넘긴다. 편집기 미리보기와 교사 테스트 모드에는 이미 다른 진입로가 있다.
- **인증은 손대지 않았다.** `/live/:code`가 이미 "현황 암호를 넣어 주세요"로 묻고, 시도 제한(10분 10회)도 그대로다. 이 링크 자체는 아무 권한도 주지 않는다 — 누가 눌러도 암호 앞에서 막힌다.

**검증**: typecheck / 테스트 **334개** 통과. 빌드 결과물에서 `시작하기` 바로 뒤에 조건부 `/live/${code}` 링크가 들어간 것을 확인했다.
⚠️ **문자열 `수업 현황 보기`만으로 반영을 판단하면 안 된다** — 홈과 결과 화면에 이미 있던 값이라 옛 번들에서도 걸린다(세션 15에서 같은 실수를 했다). `시작하기` 주변 문맥으로 봐야 한다.

**남은 확인**: 브라우저 확장이 연결돼 있지 않아 **실제 화면 클릭 확인을 못 했다.** 배포 후 폰에서 학생 QR → 진입 화면 맨 아래 링크 → 현황 암호 입력까지 한 번 밟아 볼 것.
