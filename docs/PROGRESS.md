...
**마지막 갱신**: 2026-08-05 · 세션 11 (서답형 '채점 안함' 옵션 + 결과 화면 답안 아코디언 + 제출 중 로딩 오버레이 — 아래 "세션 11" 절 참고 — **`Code.gs` 재배포 필요**: gradeShort에 매치모드 'none' 분기 추가함)
...
**다음에 할 일**: `Code.gs`를 재배포해야 라이브 백엔드에도 `gradeShort`의 `'none'` 분기가 반영된다(CLAUDE.md의 "Apps Script 재배포 절차" 참고, 어시스턴트가 배포까지 수행 가능하도록 사전 위임됨).
