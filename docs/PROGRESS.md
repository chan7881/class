...
원인은 "이 값이 편집 키인가 현황 암호인가"라는 판단이 **홈과 진행 상황 화면 두 곳에 흩어져** 있었던 것. `lib/editorAuth.ts`의 `saveLiveSecret`/`looksLikeEditToken`으로 한 곳에 모으고 양쪽이 같이 쓰게 했다(`editorAuth.test.ts` 8개로 고정). 겸사겸사 **공용 PC용 "저장 안 함"이 현황 암호에는 안 걸리던 것**도 고쳤다(암호만 남으면 다음 사용자가 그 반 답안을 본다).
