...
**마지막 갱신**: 2026-07-30 · 세션 6 (전체 블록·문항 21종 QA 통과 + 발견한 실버그 6건·UX 2건 일괄 수정, 제목 블록 서식 지정 + 2단 레이아웃 신규 기능 구현. **`apps-script/Code.gs`가 수정됐지만 아직 재배포 안 됨 — 다음 세션 첫 할 일로 처리할 것**)

## 지금 어디까지 됐나

**세션 6에서 사용자 요청으로 모든 블록·문항 유형(21종)을 담은 QA 전용 테스트 수업을 만들어 교사 기능 전체와 학생 5명 응시·제출·교사 모니터링·결과 대시보드를 훑었다.** 발견한 문제를 목록화한 뒤 한꺼번에 수정(아래 "세션 6" 절 참고), 이어서 사용자가 그 자리에서 요청한 신규 기능 2건(제목 블록 서식 지정, 블록 2단 레이아웃)까지 구현하고 전체를 다시 브라우저로 재검증했다. typecheck/test(189개, 176→189)/build 전부 통과. **`Code.gs`는 코드만 수정됐고 아직 재배포 전이다 — 다음에 이 프로젝트를 열면 가장 먼저 재배포부터 할 것** (절차는 `CLAUDE.md` "Apps Script(Code.gs) 재배포 절차" 참고).
...
**미해결·주의사항**:
- **`apps-script/Code.gs`가 수정된 채 아직 재배포 안 됨** (divToFrac 정규화 추가 + `createLesson`의 `version: 1→2`). 채점 정합성(4번 버그)과 신규 수업 생성 시 스키마 버전을 위해 다음 세션에서 가장 먼저 재배포할 것 — 배포는 공유 중앙 백엔드에 즉시 영향을 주는 작업이라 사용자에게 먼저 알리고 진행.
