...
**마지막 갱신**: 2026-07-30 · 세션 6 재검증 (URL 임베드 포함 전체 블록·문항 22종을 담은 새 QA 수업으로 회귀 테스트 재개 — 실채점 버그 1건 발견·수정, tiptap 경고 1건 수정, 죽은 코드 1건 정리, 접근성 이슈 1건은 사용자 결정으로 보류. `Code.gs` 재배포(버전 10→11)까지 완료하고 실배포에서 채점 수정이 실제로 반영됨을 재확인함 — **더 이상 남은 할 일 없음**)
