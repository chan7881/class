...
2. **결정 확인 — 관리자 "수정" 버튼이 기존 편집 링크를 무효화하는 트레이드오프**: 위 1번 항목·`docs/DECISIONS.md` "2026-07-29 관리자 화면 '수정' 버튼" 참고. 관리자가 어떤 수업을 "수정"으로 열면 그 수업 담당 교사가 저장해둔 예전 편집 링크(복구 링크 등)가 그 순간부터 동작하지 않게 된다. 이 동작이 괜찮은지, 아니면 (a) 수정 버튼을 없애고 다운로드만 유지 (b) 교사에게 알리는 장치를 추가 등 다른 방식을 원하는지 확인 필요.
3. **프런트엔드 배포**: 이번 세션 변경분은 아직 `main`에 커밋만 됐고 푸시 전이다(아래 커밋 내역 참고) — 푸시하면 GitHub Actions가 자동으로 GitHub Pages에 배포한다.
