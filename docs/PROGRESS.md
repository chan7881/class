...
**커밋·푸시 + 실사이트(live) 배포 검증**: 사용자 승인 받아 커밋(`8152cfa`) 후 `main`에 푸시 완료. GitHub Actions "Deploy to GitHub Pages" 워크플로가 자동 트리거돼 성공(`status: completed, conclusion: success`, GitHub API로 확인). **실사이트(`https://chan7881.github.io/class/`)에서 직접 재확인**: (1) 배포된 JS 번들 해시(`index-Br3XCy2p.js`)가 로컬 프로덕션 빌드 결과물과 완전히 일치 — 최신 커밋이 정확히 배포됐다는 확증. (2) 홈 화면에 "관리자" 링크가 새로 보임. (3) `/#/admin`에 임의 비밀번호를 제출하면 `"알 수 없는 action: listLessons"` 에러가 뜸 — **이건 버그가 아니라 "프런트엔드는 새 코드가 배포됐고, 백엔드(Code.gs)는 아직 옛날 버전"이라는 정확한 신호**(아래 "사용자가 해야 할 일" 1번이 아직 안 된 상태와 정확히 일치). 콘솔 에러 없음.

### 사용자가 해야 할 일 (이 세션이 대신할 수 없는 것)

1. **`apps-script/Code.gs` 재배포** — 이미지 URL 패턴 교체(6번), 데이터표 채점 코드 삭제(13번), 서답형 키워드 채점(10번), 관리자 API 3종(2번)이 전부 `Code.gs` 변경이라 재배포해야 실제 사이트에 반영된다. **"재배포"는 파일 업로드가 아니라 script.google.com 브라우저 에디터에 복사-붙여넣기다** (이 프로젝트는 `clasp` 같은 CLI 배포 도구를 안 씀 — 확인됨, 관련 설정 파일 없음). 정확한 5단계는 `CLAUDE.md`의 "Apps Script(Code.gs) 재배포 절차" 절에 있다(요약: script.google.com 프로젝트 열기 → `Code.gs` 내용 전체 지우고 로컬 `apps-script/Code.gs` 내용 복사-붙여넣기 → 저장 → **배포 관리 → 기존 배포를 새 버전으로 편집**(새 배포를 새로 만들면 URL이 바뀌니 주의) → 배포).
2. **관리자 비밀번호 등록** — 위 재배포와 별개로, script.google.com 프로젝트의 **프로젝트 설정 → 스크립트 속성**에 `ADMIN_PASSWORD`를 등록해야 `/#/admin`이 열린다(`apps-script/SETUP.md` "3-1" 절, `CLAUDE.md`에도 5번 항목으로 요약). 설정 전까지는 "관리자 비밀번호가 서버에 설정되어 있지 않습니다" 오류만 뜬다(의도된 동작). 이건 코드 배포가 아니라 설정값이라 "새 버전 배포" 없이 즉시 반영된다.

**재배포가 끝났는지 확인하는 법**(다음 세션이 이어서 할 것): `/#/admin`에 아무 비밀번호나 넣어봐서 `"알 수 없는 action"` 에러가 사라지고 실제 수업 목록이 뜨면 재배포가 반영된 것이다. 또는 실제로 사진을 업로드해 `lh3.googleusercontent.com/d/...` 형태 URL이 나오는지 확인해도 된다(예전엔 `drive.google.com/uc?export=view&id=...`였다).
